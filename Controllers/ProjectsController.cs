...
namespace TimeSheetAPI.Controllers
{
    ...
    public class ProjectsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateProject(Guid id, [FromBody] UpdateProjectModel model)
        {
            ...
            try
            {
                ...
                if (model.TeamId.HasValue)
                {
                    ...
                    // Filter teams that have this project
                    var teams = new List<Team>();
                    foreach (var team in allTeams)
                    {   
                        var teamProjects = await _teamService.GetTeamProjectsAsync(team.Id);
                        if (teamProjects.Any(p => p.Id == id))
                        {
                            teams.Add(team);
                        }
                    }
        ...
        public async Task<ActionResult<IEnumerable<ProjectLevel>>> GetProjectLevels(Guid projectId)
        {
            ...
            var levels = await _projectService.GetProjectLevelsAsync(projectId);
            return Ok(levels);
        }
        ...
        public async Task<ActionResult<IEnumerable<ProjectTask>>> GetProjectTasks(Guid levelId)
        {
            ...
            var tasks = await _projectService.GetProjectTasksAsync(levelId);
            return Ok(tasks);
        }
        ...
        public async Task<ActionResult<IEnumerable<ProjectSubtask>>> GetProjectSubtasks(Guid taskId)
        {
            ...
            var subtasks = await _projectService.GetProjectSubtasksAsync(taskId);
            return Ok(subtasks);
        }
